...
public class PublisherControllerTest {
    ...
    @Test
    public void whenGetAllPublishers_thenOK() throws Exception {
        Publisher publisher = new Publisher();
        publisher.setId("id1");
        publisher.setName("Test Publisher");

        when(_publisherService.findAll()).thenReturn(List.of(publisher));

        _mockMvc.perform(get("/publishers").contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$.[0].name", is("Test Publisher")));
    }

    @Test
    public void whenGetPublisher_thenOK() throws Exception {
        Publisher publisher = new Publisher();
        publisher.setId("id1");
        publisher.setName("Test Publisher");

        when(_publisherService.getPublisher("id1")).thenReturn(publisher);

        _mockMvc.perform(get("/publishers/id1").contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is("id1")))
                .andExpect(jsonPath("$.name", is("Test Publisher")));
    }

    @Test
    @WithMockUser(username = "mock_user", roles = {"crud_comics"})
    public void givenAuthorizedUser_whenInsertPublisher_thenOK() throws Exception {
        Publisher publisher = new Publisher();
        publisher.setName("New Publisher");

        Publisher inserted = new Publisher();
        inserted.setId("new_id");
        inserted.setName("New Publisher");
        MetaData meta = new MetaData();
        meta.setCreatedOn(new Date());
        meta.setCreatedBy("mock_user");
        inserted.setMetaData(meta);

        when(_publisherService.insert(any(), any())).thenReturn(inserted);

        _mockMvc.perform(post("/publishers")
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(_objectMapper.writeValueAsString(publisher)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is("new_id")))
                .andExpect(jsonPath("$.name", is("New Publisher")));
    }

    @Test
    @WithMockUser(username = "mock_user", roles = {"crud_comics"})
    public void givenAuthorizedUser_whenSavePublisher_thenOK() throws Exception {
        Publisher publisher = new Publisher();
        publisher.setId("id1");
        ...
        when(_publisherService.save(any(), any())).thenReturn(saved);

        _mockMvc.perform(put("/publishers/id1")
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(_objectMapper.writeValueAsString(publisher)))
                .andExpect(status().isOk())
